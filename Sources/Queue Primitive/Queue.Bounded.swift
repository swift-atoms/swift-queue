...
extension __Queue where S: __ColumnDirect, S: ~Copyable {
    ...
    /// `__Queue<S.BoundedTwin>` — so it is **column-PRESERVING**: for the default ring column
    /// `S.BoundedTwin` is `Buffer.Ring.Bounded` (the ring's own bounded twin), and for any
    ...
    /// the alias body is generic over `S.BoundedTwin`, so it names no concrete bounded type
    ...
    public typealias Bounded = __Queue<S.BoundedTwin>
}
